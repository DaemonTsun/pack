...
void generate_header(arguments *args)
{
    ...
    if (fs::exists(opath))
    {
        if (!fs::is_regular_file(opath))
            throw std::runtime_error(str("not a writable file: ", opath));
    ...
    if (!open(&stream, args->out_path, MODE_WRITE, false, false))
        throw std::runtime_error(str("could not open ", opath, " for writing"));
...
try
{
    ...
    if (action_count > 1)
        throw std::runtime_error("can only do one of extract (-x), generate header (-g) or list (-l)");
